...
class WFSPB_F_FrontEnd {
	...
	public function enqueue_script_frontend() {
		...
		if ( $enable && $params->check_woo_shipping_zone() ) {
			wp_enqueue_style( 'woo-free-shipping-bar', plugins_url( 'woo-free-shipping-bar/assets/css/woo-free-shipping-bar-frontend-style.css', 'woo-free-shipping-bar'), array(), WFSPB_F_VERSION  );
			...
			if ( ! empty( $font ) ) {
				...
				wp_enqueue_style( 'google-font-' . strtolower( $font ), '//fonts.googleapis.com/css?family=' . $font . ':400,700', array(), WFSPB_F_VERSION );
			}

			switch ( $style ) {
				case 2:
					wp_enqueue_style( 'woo-free-shipping-bar-style2', plugins_url( 'woo-free-shipping-bar/assets/css/style-progress/style2.css', 'woo-free-shipping-bar' ), array(), WFSPB_F_VERSION );
					$css_style2 = "
					...
					break;
				case 3:
					wp_enqueue_style( 'woo-free-shipping-bar-style3', plugins_url( 'woo-free-shipping-bar/assets/css/style-progress/style3.css', 'woo-free-shipping-bar' ), array(), WFSPB_F_VERSION );
					break;
				default :
					wp_enqueue_style( 'woo-free-shipping-bar-style', plugins_url( 'woo-free-shipping-bar/assets/css/style-progress/style.css', 'woo-free-shipping-bar' ), array(), WFSPB_F_VERSION );
					break;
			}
			...
			if ( $enable_progress ) {
				...
				$is_not_int = preg_match('/[^0-9]/', $order_min_amount );
				if ( $is_not_int ) {
					$order_min_amount = preg_replace('/\D/', '', $order_min_amount);
					$order_min_amount = substr_replace( $order_min_amount, '.', strlen( $order_min_amount ) - wc_get_price_decimals(), 0 );
				}
				...
				if ( $total >= $order_min_amount ) {
					...
					if ( $order_min_amount == 0 ) {
						...
						$amount_total_pr = round( ( $total * 100 ) / $order_min_amount, wc_get_price_decimals() );
					}
					$custom_css .= "
	...
	public function show_bar() {
		...
		$message_ss = str_replace( '{checkout_page}', $checkout, '<div id="wfspb-main-content">' . wp_strip_all_tags( wp_unslash( $message_success ) ) . '</div>' );
		...
		if ( is_checkout() ) {
			if ( $total < $order_min_amount ) {
				...
				$message           = str_replace( $key_msgerror, $msgerror_replaced, '<div id="wfspb-main-content">' . wp_strip_all_tags( wp_unslash( $message_error ) ) . '</div>' );
			} else {
				$message = $message_ss;
			}
		} else {
			if ( $total < $order_min_amount ) {
				...
				$message = str_replace( $key, $replaced, '<div id="wfspb-main-content">' . wp_strip_all_tags( wp_unslash( $message_purchased ) ) . '</div>' );
			} else {
				$message = $message_ss;
			}
		}
        ...
        <div id="wfspb-top-bar" class="displaying customized <?php echo esc_attr( $class_pos ) ?>"
			...
			if ( $total == 0 ) {
				$message = str_replace( $announce_min_amount, wc_price( $order_min_amount ), '<div id="wfspb-main-content">' . wp_strip_all_tags( wp_unslash( $announce_system ) ) . '</div>' );
			}
			...
			if ( $enable_progress ) {
                ...
                <div id="wfspb-progress" class="<?php echo esc_attr( implode( ' ', $class ) ) ?>">
                    <div id="wfspb-current-progress"
                         style="<?php echo esc_attr( intval( $current_percent ) > 0 ? 'width:' . $current_percent . '%' : '' ) ?>">
